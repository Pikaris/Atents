...
public class ObjectPool<T> : MonoBehaviour where T : RecycleObject  // T는 반드시 RecycleObject를 상속받은 오브젝트다
{
    ...
    /// <summary>
    /// 현재 사용가능한 오브젝틀들을 관리하는 큐(pool배열에서 비활성화 되어 있는 오브젝트만 들어있는 자료구조)
    /// </summary>
    Queue<T> readyQueue;
    ...
    void GenerateObjects(int start, int end, T[] result)
    {
        for(int i = start; i < end; i++)
        {
            ...
            obj.name = $"{prefab.name}_{i}";                    // 이름 변경(숫자 이용해서 구분하기)

            T comp = obj.GetComponent<T>();
            comp.onDisable += () =>                             // 람다 함수를 comp의 onDisable 델리게이트에 등록 = comp가 비활성화 되면 아래 코드가 실행된다.
            {
                readyQueue.Enqueue(comp);                       // 레디큐에 컴포넌트 추가해 놓기
            };
            OnGenerateObject(comp);

            result[i] = comp;       // 배열에 만들어진 것을 모두저장
            obj.SetActive(false);   // 비활성화 시키기
        }
    }

    /// <summary>
    /// 오브젝트 하나가 생성되었을 때 실행되는 함수(빈 함수)
    /// </summary>
